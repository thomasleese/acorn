...
TEST(parser_token_test, token_kind_string_for_keyword)
{
    Token token;
    token.kind = Token::Keyword;
    token.lexeme = "test";
    EXPECT_EQ(token.kind_string(), "test");
}

TEST(parser_token_test, token_kind_string_for_other)
{
    Token token;
    token.kind = Token::Name;
    token.lexeme = "test";
    EXPECT_EQ(token.kind_string(), "name");
}
